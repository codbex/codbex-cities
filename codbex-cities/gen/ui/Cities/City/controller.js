angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', function ($scope, $http, messageHub, entityApi) {
		...
		//-----------------Custom Actions-------------------//
		$http.get("/services/js/resources-core/services/custom-actions.js?extensionPoint=codbex-cities-custom-action").then(function (response) {
			$scope.pageActions = response.data.filter(e => e.perspective === "Cities" && e.view === "City" && (e.type === "page" || e.type === undefined));
			$scope.entityActions = response.data.filter(e => e.perspective === "Cities" && e.view === "City" && e.type === "entity");
		});

		$scope.triggerPageAction = function (actionId) {
			for (const next of $scope.pageActions) {
				if (next.id === actionId) {
					messageHub.showDialogWindow("codbex-cities-custom-action", {
						src: next.link,
					});
					break;
				}
		...
		$scope.loadPage = function (pageNumber) {
			...
			entityApi.count().then(function (response) {
				...
				entityApi.list(offset, limit).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("City", `Unable to list City: '${response.message}'`);
						return;
					}
